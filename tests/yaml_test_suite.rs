...
#[test]
fn from_str_agrees_with_the_suite_on_every_single_document_case() {
    let Some(dir) = suite_dir() else { return };
    let cases = load_cases(&dir);
    let (mut passed, mut multi, mut refused) = (0usize, 0usize, 0usize);
    let mut failures = Vec::new();
    for case in &cases {
        let expected = expected_docs(case);
        let is_multi = expected.as_ref().is_some_and(|d| d.len() != 1);
        match noyalib_serde_yaml::from_str::<noyalib_serde_yaml::Value>(&case.yaml) {
            ...
            Ok(v) => match expected.as_deref().and_then(<[_]>::first) {
                Some(exp) if !json_eq(&to_json(&v), exp) => {
                    failures.push(format!("{}: {} != {exp}", case.id, to_json(&v)));
                }
                _ => passed += 1,
            },
